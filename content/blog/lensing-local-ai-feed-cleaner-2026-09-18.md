...
There's also a thumbs-up/thumbs-down option, off by default. A thumb never moves your topic, that score stays the last word, what it does instead is narrower: rate a post and the next repost, quote, or same story told the same way, follows your call instead of getting judged from scratch, everything else keeps going purely by score, like you never touched a thumb. There's no keyword extraction hiding in there to inspect, the post goes in as one string and comes back as one vector, so a correction lives on the topic line it shaped, and rating the same post again just undoes it.
