...
There's also a thumbs-up/thumbs-down option, off by default. Your topic score stays the source of truth, a thumb never moves it. What it does instead is narrower: rate a post and near-identical posts on that same topic line, a repost, a quote, the same story told the same way, follow your rating, while everything else is still judged purely on score. There's no keyword extraction hiding in there to inspect, the post goes in as one string and comes back as one vector, so a correction lives on the topic line it shaped, and rating the same post again just undoes it.
